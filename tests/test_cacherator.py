...
class TestSaveOnDel:
    """Test suite for save_on_del parameter."""
    ...
    def _make_cache(self, save_on_del):
        class TestClass(JSONCache):
            def __init__(self):
                super().__init__(data_id="test_save_on_del", directory="test_cache", save_on_del=save_on_del)
        obj = TestClass()
        obj._dynamodb_enabled = True
        obj._write_to_dynamodb_calls = 0
        obj._json_cache_save_l1_calls = 0
        original_write = obj._write_to_dynamodb
        original_l1 = obj._json_cache_save_l1
        obj._write_to_dynamodb = lambda: (setattr(obj, "_write_to_dynamodb_calls", obj._write_to_dynamodb_calls + 1) or None)
        obj._json_cache_save_l1 = lambda: (setattr(obj, "_json_cache_save_l1_calls", obj._json_cache_save_l1_calls + 1) or original_l1())
        return obj

    def test_save_on_del_default_is_false(self):
        class TestClass(JSONCache):
            def __init__(self):
                super().__init__(data_id="test_default", directory="test_cache")
        obj = TestClass()
        assert obj._save_on_del is False

    def test_finalize_skips_dynamodb_by_default(self):
        obj = self._make_cache(save_on_del=False)
        obj._finalize_cache()
        assert obj._write_to_dynamodb_calls == 0

    def test_finalize_writes_dynamodb_when_save_on_del_true(self):
        obj = self._make_cache(save_on_del=True)
        obj._finalize_cache()
        assert obj._write_to_dynamodb_calls == 1

    def test_finalize_always_saves_l1(self):
        for save_on_del in (False, True):
            obj = self._make_cache(save_on_del=save_on_del)
            obj._finalize_cache()
            assert obj._json_cache_save_l1_calls == 1, f"L1 not saved when save_on_del={save_on_del}"

    def test_json_cache_save_always_writes_both(self):
        obj = self._make_cache(save_on_del=False)
        obj.json_cache_save()
        assert obj._json_cache_save_l1_calls == 1
        assert obj._write_to_dynamodb_calls == 1
